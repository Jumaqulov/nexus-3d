export function initializeHero(scope) {
  "use strict";
  ...
  const worldData = {
    rift: {
      ...
      caption: "EXPEDITION_001",
      color: [216, 225, 146],
    },
    eden: {
      title: "EDEN",
      subtitle: "WHERE THE WILD TAKES OVER",
      caption: "EXPEDITION_002",
      color: [126, 210, 196],
    },
  };
  let world = "rift";
  ...
  const particles = Array.from(
    { length: touchMedia.matches ? 52 : 115 },
    () => ({
      x: (Math.random() - 0.5) * 2000,
      y: (Math.random() - 0.5) * 1450,
      z: Math.random() * 1400 + 100,
      radius: Math.random() * 1.35 + 0.3,
      speed: Math.random() * 0.13 + 0.045,
  ...
  function selectWorld(next, announceChange = true) {
    ...
    document.querySelector("#immersion-title").textContent =
      worldData[next].title;
    document.querySelector("#immersion-world-name").textContent =
      worldData[next].title;
    document.querySelector("#immersion-subtitle").textContent =
      worldData[next].subtitle;
    document.querySelector(".scene-caption span:first-child").textContent =
      worldData[next].caption;
  ...
  document
    ...
    .forEach((button) =>
      scope.listen(button, "click", () => selectWorld(button.dataset.world)),
    );
  function enterImmersion(next, source) {
    ...
    window.dispatchEvent(
      new CustomEvent("nexus:immersionchange", { detail: { active: true } }),
    );
    announce(
      "Immersion opened. " +
        worldData[world].title +
  ...
  function exitImmersion() {
    ...
    resize();
    window.dispatchEvent(
      new CustomEvent("nexus:immersionchange", { detail: { active: false } }),
    );
    announce("Returned to NEXUS");
  }
  document
    .querySelectorAll("[data-immerse]")
    .forEach((button) =>
      scope.listen(button, "click", () => enterImmersion(world, button)),
    );
  document
    .querySelectorAll("[data-open-world]")
    .forEach((button) =>
      scope.listen(button, "click", () =>
        enterImmersion(button.dataset.openWorld, button),
      ),
    );
  scope.listen(
    document.querySelector(".exit-immersion"),
    "click",
    exitImmersion,
  );
  scope.listen(document.querySelector(".next-world"), "click", () =>
  ...
  scope.listen(document, "keydown", (event) => {
    ...
    if (event.key === "Tab") {
      const focusable = [
        ...immersionUI.querySelectorAll("button"),
        motionButton,
      ];
  ...
  function setMotion(enabled) {
    window.dispatchEvent(
      new CustomEvent("nexus:beforemotionchange", { detail: { enabled } }),
    );
    motion = enabled;
    ...
    motionButton.querySelector("span").textContent = motion
      ? "Motion on"
      : "Motion off";
    motionButton
      .querySelector("use")
      .setAttribute("href", motion ? "#icon-pause" : "#icon-play");
    if (!motion) {
    ...
    window.dispatchEvent(
      new CustomEvent("nexus:motionchange", { detail: { enabled: motion } }),
    );
  }
  scope.listen(motionButton, "click", () => setMotion(!motion));
  scope.listen(motionMedia, "change", (event) => setMotion(!event.matches));
  ...
  scope.listen(window, "pointermove", updatePointer, { passive: true });
  scope.listen(document, "pointerleave", () => {
    pointer = { x: 0, y: 0 };
  });
  scope.listen(
    window,
  ...
  if (touchMedia.matches) {
    document.querySelector(".immersive-hint").textContent =
  ...
  scope.listen(window, "resize", resize, { passive: true });
  scope.resizeObserver(resize).observe(hero);
  function drawParticles() {
    ...
    const focalLength = width * 0.7;
    particles.forEach((p) => {
      const perspective = focalLength / (focalLength + p.z);
      const x = (p.x - viewX) * perspective * (width / 1300) + width * 0.62;
      const y = (p.y - viewY) * perspective * (height / 900) + height * 0.5;
      const alpha = Math.max(
        0.08,
        (1 - p.z / 1650) * (0.4 + Math.sin(time * 0.0007 + p.phase) * 0.15),
      );
      ctx.beginPath();
      ctx.arc(x, y, Math.max(0.35, p.radius * perspective), 0, Math.PI * 2);
  ...
  function animate(now) {
    ...
    lastFrame = now;
    time += delta;
    eased.x += (pointer.x - eased.x) * 0.047;
    eased.y += (pointer.y - eased.y) * 0.047;
    const drift = Math.sin(time * 0.00018) * 3;
    const scroll = immersed ? 0 : Math.min(scrollY, height) * 0.26;
    const journeyProgress = immersed
      ? 0
      : Math.min(scrollY / Math.max(height, 1), 1);
    const amplitude = immersed ? 2.1 : 1.3;
    const bgTransform = `translate3d(${-eased.x * 21 * amplitude + drift}px,${-eased.y * 15 * amplitude + scroll}px,0) scale(${1.08 + journeyProgress * 0.36})`;
    landscape.style.transform = bgTransform;
    eden.style.transform = bgTransform;
    character.style.transform = `translate3d(${-eased.x * 43 * amplitude}px,${-eased.y * 25 * amplitude - scroll * 0.42 + Math.sin(time * 0.00055) * 4}px,0) rotateY(${eased.x * -4}deg) scale(${1 + journeyProgress * 0.19})`;
    document.querySelector(".hero-copy").style.transform =
      `translateY(${journeyProgress * height * 0.12}px)`;
    document.querySelectorAll(".hero h1>span").forEach((line, index) => {
      line.style.transform = `translateX(${journeyProgress * [40, -32, 55][index]}px)`;
    });
    for (const p of particles) {
      ...
      if (p.z < 25) {
        p.z = 1500;
        p.y = (Math.random() - 0.5) * 1450;
      }
    }
    drawParticles();
    frame = scope.requestFrame(animate);
  }
  ...
  function initAudio() {
    ...
    for (let i = 0; i < length; i++) {
      last = (last + Math.random() * 0.03 - 0.015) / 1.018;
      samples[i] = last;
    }
  ...
  async function toggleSound() {
    try {
      if (!audio && !initAudio()) {
        announce("Ambient audio is not available in this browser");
        return;
      }
      ...
      document
        ...
        .setAttribute("aria-pressed", String(soundOn));
      document
        .querySelector(".sound-toggle")
        .setAttribute(
          "aria-label",
      ...
      immersiveSound.firstChild.textContent = soundOn
        ? "SOUND ON "
        : "SOUND OFF ";
      announce("Ambient sound " + (soundOn ? "on" : "off"));
    } catch {
      if (scope.active)
        announce("Audio could not start. Try enabling sound again.");
    }
  }
  scope.listen(document.querySelector(".sound-toggle"), "click", toggleSound);
  scope.listen(
    document.querySelector(".immersion-sound"),
    "click",
    toggleSound,
  );
  scope.listen(document, "visibilitychange", () => {
    if (document.hidden) {
      if (frame !== null) scope.cancelFrame(frame);
      frame = null;
